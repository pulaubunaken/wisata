...
function renderKatalogPaging() {
    ...
    dataHalamanIni.forEach(umkm => {
        ...
        const wa = umkm.nomor_wa ? `https://wa.me/${umkm.nomor_wa}?text=Halo%20${encodeURIComponent(nama)},%20saya%20melihat%20usaha%20Anda%20dari%20Info%20Wisata%20Bunaken.` : '#';
        ...
        wadahKatalog.innerHTML += `
            <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 flex flex-col justify-between hover:shadow-lg transition-shadow">
                ...
                <div class="p-5 pt-0 flex gap-2">
                    <a href="detail.html?id=${umkm.user_id}" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-center py-2.5 rounded-lg font-semibold text-sm transition-colors shadow">
                        🔍 Lihat Detail
                    </a>
                    ${umkm.nomor_wa ? `<a href="${wa}" target="_blank" class="bg-green-500 hover:bg-green-600 text-white px-3 py-2.5 rounded-lg font-semibold text-sm transition-colors shadow flex items-center justify-center">💬 WA</a>` : ''}
                </div>
            </div>
        `;
    });
...
function tambahBarisGaleri(urlFoto = '') {
    ...
    if (jumlahBaris >= 3 && !urlFoto) {
        alert("Maksimal galeri adalah 3 foto.");
        return;
    }
...
// Fungsi Unduh Stiker QR Lengkap dengan Nama Usaha & Teks Panduan
...
async function loadDetailUMKM() {
    ...
    if (data.nomor_wa) {
        ...
        btnWa.href = `https://wa.me/${data.nomor_wa}?text=${encodeURIComponent(teksPesan)}`;
    ...
    if (wadahGaleri) {
        ...
        if (data.galeri_foto && data.galeri_foto.length > 0) {
            data.galeri_foto.forEach(link => {
                wadahGaleri.innerHTML += `<img src="${link}" onclick="bukaLightbox('${link}')" class="w-full h-24 object-cover rounded-lg shadow border border-gray-200 cursor-pointer hover:opacity-75 transition-opacity duration-200" alt="Galeri">`;
            });
        } else {
            wadahGaleri.innerHTML = '<p class="text-xs text-gray-500 col-span-3">Belum ada foto galeri.</p>';
        }
    }
}
